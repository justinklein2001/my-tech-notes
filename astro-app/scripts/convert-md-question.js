...
const OUTPUT_FILE = "raw/private/leetcode.json";

// The raw markdown you provided
...
async function main() {
  ...
  // Regex to match [Title](URL) where URL contains "leetcode.com/problems/"
  const regex = /\[([^\]]+)\]\((https:\/\/leetcode\.com\/problems\/[^)]+)\)/g;
  
  const questions = [];
  let match;

  while ((match = regex.exec(MARKDOWN_SOURCE)) !== null) {
    questions.push({
      title: match[1],
      url: match[2]
    });
  ...
  console.log(`✅ Extracted ${questions.length} LeetCode questions.`);
